...
public class UserRepository implements IModel {
    ...
    public Observable<List<Time>> getTimen(int user_id,int lastIdQueried, boolean update) {
        //使用rxcache缓存,上拉刷新则不读取缓存,加载更多读取缓存
        return Observable.just(mManager
                .createRetrofitService(UserService.class)
                .getTimen(user_id,lastIdQueried, USERS_PER_PAGE))
                .flatMap(new Function<Observable<List<Time>>, ObservableSource<List<Time>>>() {
                    @Override
                    public ObservableSource<List<Time>> apply(@NonNull Observable<List<Time>> listObservable) throws Exception {
                        return mManager.createCacheService(CommonCache.class)
                                .getTime(listObservable
                                        , new DynamicKey(lastIdQueried)
                                        , new EvictDynamicKey(update))
                                .map(listReply -> listReply.getData());
                    }
